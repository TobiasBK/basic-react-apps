...
class FullHouse extends Rule {
  //extends rule from above to take advatage of freq()
  evalRoll = (dice) => {
...
class SmallStraight extends Rule {
  evalRoll = (dice) => {
    ...
    //straight is 2,3,4 + 1 OR 5
    if (d.has(2) && d.has(3) && d.has(4) && (d.has(1) || d.has(5)))
      return this.score;
    //OR straight has 3,4,5 + 2 OR 6
    if(d.has(3) && d.has(4) && d.has(6) && (d.has(2) || d.has(6)))
      return this.score;
...
const fullHouse = new FullHouse({count: 25});

// small/large straights score as 30/40
const smallStraight = new SmallStraight({count: 30})
